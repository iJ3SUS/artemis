    ...
    <Page>
        <template #heading>
            <Heading :sticky="true">
                ...
                <template #title>
                    <div>
                        <h1 class="text-2xl font-semibold text-gray-900">Editar permiso</h1>
                        <p class="text-sm text-gray-600 mt-0.5">Modifica los datos del permiso</p>
                    </div>
                </template>
                <template #actions>
...
const { form, errors, fill, submit } = useForm(PermissionSchema)
...
const load = async () => {
    ...
    const { success, response } = await http.request({
        method: 'GET',
        url: `dashboard/permissions/${route.params.id}`
    })
