<template>
    <div class="min-h-screen w-full relative bg-background flex items-center justify-center p-4 overflow-y-auto overflow-x-hidden">
        ...
        <div class="relative w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-0 md:gap-6 my-auto">
            <!-- Left panel - Branding -->
            <div class="relative overflow-hidden order-2 md:order-1 rounded-b-2xl md:rounded-2xl bg-gradient-to-br from-primary-600 via-primary-700 to-primary-900 text-white p-6 md:p-8 shadow-2xl flex flex-col justify-center min-w-0">
                <div class="absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg%20width%3D%2260%22%20height%3D%2260%22%20viewBox%3D%220%200%2060%2060%22%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%3E%3Cg%20fill%3D%22none%22%20fill-rule%3D%22evenodd%22%3E%3Cg%20fill%3D%22%23ffffff%22%20fill-opacity%3D%220.05%22%3E%3Cpath%20d%3D%22M36%2034v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6%2034v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6%204V0H4v4H0v2h4v4h2V6h4V4H6z%22%2F%3E%3C%2Fg%3E%3C%2Fg%3E%3C%2Fsvg%3E')] opacity-30" />
                <div class="relative">
                    <div>
                        <h2 class="text-2xl md:text-3xl font-bold leading-tight">{{ appName }}</h2>
                        <p class="mt-1 text-primary-100/80">Administra tu equipo desde un solo lugar.</p>
                    </div>
                    ...
                    <div class="mt-5 grid grid-cols-2 gap-3">
                        <div class="rounded-xl bg-white/10 backdrop-blur-sm p-3 ring-1 ring-white/20">
                            <div class="text-xs text-primary-100/70">Colaboradores</div>
                            <div class="mt-1 text-lg font-semibold">Centralizados</div>
                        </div>
                        <div class="rounded-xl bg-white/10 backdrop-blur-sm p-3 ring-1 ring-white/20">
                            <div class="text-xs text-primary-100/70">Procesos</div>
                            <div class="mt-1 text-lg font-semibold">Automatizados</div>
                        </div>
                    </div>
                </div>
            ...
            <!-- Right panel - Form -->
            <div class="bg-white order-1 md:order-2 rounded-t-2xl md:rounded-2xl shadow-2xl p-6 md:p-8 ring-1 ring-gray-200 min-w-0 flex flex-col justify-center">
                ...
                <form @submit.prevent="handleSubmit" class="space-y-4">
                    ...
                    <Button color="primary" :disabled="loading || !form.email" @handle="handleSubmit" class="w-full">
