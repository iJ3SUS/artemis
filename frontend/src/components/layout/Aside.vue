    ...
    <Teleport to="body">
        <Transition name="aside">
            <div v-if="ui.menu_open" class="fixed inset-0 z-50 lg:hidden">
                ...
                <aside class="relative h-full w-72 bg-surface border-r border-border flex flex-col">
                    <!-- Company Card -->
                    <div class="border-b border-border shrink-0">
                        <div class="flex items-center justify-between px-4 py-3">
                            <div class="flex items-center gap-3 p-3 rounded-xl bg-surface-hover border border-border/50 flex-1 mr-2">
                                <div class="w-9 h-9 rounded-lg bg-gradient-to-br from-emerald-400 to-emerald-600 flex items-center justify-center shrink-0 shadow-md shadow-emerald-600/20">
                            ...
                            <button @click="ui.menu_open = false" class="p-2 rounded-lg hover:bg-surface-hover transition-colors shrink-0">
                                <Icon icon="Close" width="18" height="18" class="text-text-muted" />
                            </button>
                        </div>
                    </div>
                    ...
                    <nav class="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
                        ...
                        <div class="mb-4">
                            ...
                            <button v-for="item in mainItems" :key="item.path" @click="navigate(item.path)" :class="[
                        ...
                        <div class="mb-4">
                            ...
                            <button v-for="item in managementItems" :key="item.path" @click="navigate(item.path)" :class="[
                                'w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm transition-all text-left',
                                route.path === item.path ? 'bg-primary-600/10 text-primary-400 font-medium' : 'text-text-muted hover:bg-surface-hover hover:text-text'
                            ]">
                                <Icon :icon="item.icon" width="18" height="18" />
                    ...
                    <div class="border-t border-border p-3 shrink-0">
                        <div class="flex items-center gap-3 px-2 py-2 rounded-lg bg-surface-hover">
                            ...
                            <button @click="handleLogout" class="p-1.5 rounded hover:bg-background transition-colors shrink-0">
...
const mainItems = computed(() => menuItems.filter(item => item.meta?.protected && item.meta?.section === 'main'))
const managementItems = computed(() => menuItems.filter(item => item.meta?.protected && item.meta?.section === 'management'))
